...
void mute_binding(struct weston_keyboard *keyboard, const struct timespec *time, uint32_t key, void *data)
{
	setup_shm(SHM_MUTE);
}
...
void exposay_binding(struct weston_keyboard *keyboard, const struct timespec *time, uint32_t key,
					 void *data)
{
	struct desktop_shell *shell = data;
...
int is_shell_surface(struct weston_surface *surface)
{
...
get_default_view(struct weston_surface *surface)
{
	...
	*/
	...
	wl_list_for_each(view, &surface->views, surface_link) if (weston_view_is_mapped(view)) return view;
			...
			global_exposay.row_current < (global_exposay.cur_output->grid_size - 1))
		{
			exposay_maybe_move(shell, global_exposay.row_current + 1,
							   (global_exposay.cur_output->num_surfaces %
								global_exposay.cur_output->grid_size) -
								   1);
		}
		break;
	case KEY_LEFT:
